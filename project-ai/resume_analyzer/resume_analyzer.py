"""Core resume analysis: ATS scoring, quality scoring, section feedback, and skills gap."""
...
def _compute_skills_gap(
    ...
    """Computes skills gap via regex matching. Returns matched, missing, and nice-to-have missing lists."""
...
def _call_llm_for_analysis(
    ...
    """Makes one Groq API call for ATS scores, quality score, and section feedback. Returns parsed dict."""
...
def _build_safe_analysis(llm_data: dict) -> dict:
    """Validates LLM response and fills in safe fallbacks for missing or malformed fields."""
...
def analyze_resume(parsed_resume: dict, role_id: str) -> dict:
    """Analyzes a parsed resume against a specific job role. Returns full analysis dict or error."""
...
def generate_upgrade_tip(all_role_results: list, parsed_resume: dict) -> dict:
    """Generates a consolidated resume upgrade tip from all role results. Returns {"upgrade_tip": str}."""
